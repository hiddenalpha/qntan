...
#define INCGUARD_aM91R0ljMWoqtn7F
  ...
  Doc comments for structs FOLLOW the top of the structs body. Doc comments
  for members FOLLOW its signature declaration. Eg:

    struct Foo {
        /* Doc for 'Foo' goes here.
         */
        int (*myFunc)( int x );
        /* Here goes description of 'myFunc'.
         */
    };
...
struct Qntan_IoMux {
	 ...
	 *
	 * @param ptrToFILE
	 *      Is a `FILE*` (NOT FileDescriptor!). In most cases you'll need a
	 *      cast to uintptr_t.
	 * @param onDone
	 ...
	 *
	 * @param fd
	 *      FileDescriptor (NOT 'FILE*'!). (Or 'HANDLE' on some ugly platforms).
	 * @param onDone
	 *      Called as soon operation has completed.
	 * @param ret
	 *      Negative numbers indicate errors. Positive values indicate
	 ...
	 * @param sockaddr, sockaddr_len
	 *      Address of the new client. WARN: Only valid if it got
	 *      requested via 'flgs'. This is only valid as long 'onDone'
...
struct Qntan_File {
	...
	void (*truncate)( struct Qntan_File**, int64_t size,
		void(*onDone)(int,Qntan_Cls), Qntan_Cls );
	/* TODO doc
	 * WARN: size MUST be zero. Any other value is UB! (aka reserved for
...
#ifndef Qntan_FourierTransform_transform
#	define Qntan_FourierTransform_transform(A, ...) (*A)->transform(A, __VA_ARGS__)
#endif

struct Qntan_FourierTransform {
	/**/
	int (*transform)( struct Qntan_FourierTransform**, float*samples, size_t samples_cnt, float*output );
	/* WARN: There may be implementations producing odd results if
	 * 'samples' and 'output' overlap.
	 * @return negative values on erorr. Zero on success.
	 */
};
...
struct Qntan_Process {
	 ...
	 *
	 * (HINT: Do not confuse 'signal' and 'kill' names here)
	 *
...
struct Qntan_JsonTreeDec_JsonNode {
	 ...
	 * NOT codepoints.
	 */
	union /* See 'type' to know which of those values is valid. */ {
		double valNumber;
		int valBool;
		struct/*valString*/{
			char const *valStr;
